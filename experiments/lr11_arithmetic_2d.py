# ==============================================
# Eksperimen #011: Low-Rank Arithmetic Solver (2-digit)
# ==============================================
...
def generate_arithmetic_data(n_samples=60000):
    a=np.random.randint(0,100,(n_samples,)); b=np.random.randint(0,100,(n_samples,)); c=a+b
    a1,a2=a//10,a%10; b1,b2=b//10,b%10
    c1,c2,c3=c//100,(c%100)//10,c%10
    return np.stack([a1,a2,b1,b2],axis=1), np.stack([c1,c2,c3],axis=1)

X,Y=generate_arithmetic_data(60000)
...
class ArithmeticMLP(nn.Module):
    def __init__(self,hidden_dims=[256,256],use_lowrank=False,rank=16):
        super().__init__()
        self.embed=nn.Embedding(10,32); in_dim=4*32
        layers=[]
        prev_dim=in_dim
...
def train_model(model,epochs=30,lr=1e-3):
    ...
    for ep in range(epochs):
        ...
        for x,y in train_loader:
            ...
            logits=model(x); loss=sum(crit(logits[:,i,:],y[:,i]) for i in range(3))
...
configs={
    "Regular 256": (ArithmeticMLP([256,256],False), "blue"),
    "LowRank r=16": (ArithmeticMLP([256,256],True,16), "orange"),
    "LowRank r=32": (ArithmeticMLP([256,256],True,32), "green"),
    "Regular 80": (ArithmeticMLP([80,80],False), "red"),
}
...
axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('Full-Sequence Accuracy'); axes[0].set_title('Arithmetic Accuracy'); axes[0].legend(); axes[0].grid(alpha=0.3)
for name,(_,color) in configs.items():
...
plt.tight_layout(); plt.savefig('exp011_arithmetic_lowrank.png'); plt.show()
